...
const PROFILES_STRUCTURE =
  "name:Profile{color:yellow},gpus:GPU[name:Name{width:16;color:cyan},power:Power,fan:Fan,core:Core,memory:Mem," +
  "lgc:LGC,lmc:LMC,hashrate:HR{color:green}]";

const OVERCLOCK_STRUCTURE =
  "index:GPU,name:Name{color:cyan;width:16},profile:Profile," +
  "power:Power[current:Cur,config:Cfg{color:magenta}],fan:Fan[current:Cur,config:Cfg{color:magenta}]," +
  "core:Core[current:Cur,config:Cfg{color:magenta}],memory:Mem[current:Cur,config:Cfg{color:magenta}]," +
  "lgc:LGC[current:Cur,config:Cfg{color:magenta}],lmc:LMC[current:Cur,config:Cfg{color:magenta}]";
...
module.exports = { displayGpuStatusTable, displayGpuChangelog, displayProfiles, displayOverclock, copyright };
