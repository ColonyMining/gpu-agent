...
const GPU_STATUS_TABLE_STRUCTURE =
  "index:GPU,pci:PCI,name:Name{color:cyan;width:16},utilization:%(gpu),power:Power[usage:Usage{color:red},limit:Limit]," +
  "temperature:Temperature,fans:Fans[index:Fan,speed:Speed]," +
...
function displayGpuStatusTable(data) {
  ...
  structure.find(item => item.field === "temperature").style = properties => text => {
...
module.exports = { displayGpuStatusTable };
