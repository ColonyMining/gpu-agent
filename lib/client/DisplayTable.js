...
const GPU_STATUS_TABLE_STRUCTURE =
  ...
  "overclock:Overclock[core:Core,memory:Mem,lgc:LGC,lmc:LMC]";
...
function displayGpuChangelog(data) {
  ...
  structure.find(item => item.field === "temperature").style = () => temperatureStyle;
...
function displayGpuStatusTable(data) {
  ...
  structure.find(item => item.field === "utilization").style = () => utilizationStyle;
  structure.find(item => item.field === "temperature").style = () => temperatureStyle;
