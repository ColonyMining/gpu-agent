...
const Client = {
  status: function (params) {
    executeCommand(StatusReport.status(params.gpus, isTrue(params.watch), params.interval, params.display));
  },
  ...
  startAgent: function (params) {
    ...
    const agent = new AgentClient(interval, temperature, isTrue(params.silent), params.display);
  ...
  saveOverclock: function (params) {
    const hashrate = params.hr ? parseInt(params.hr) : undefined;
    executeCommand(OverclockClient.save(params.gpus, params.profile, hashrate, params.display));
  },
