...
class TemperatureAnalyzer {
  constructor(defaultTemperature, display) {
  ...
  async analyze(gpus) {
    const config = new Config(this.display);
    const gpusConfig = config.get().gpus || {};

    const timestamp = moment().utc();

    const gpuFan = new Gpu(this.display).fans();
    ...
    gpus.forEach(gpu => {
      ...
      const temperature = gpuConfig ? gpuConfig.temperature || this.defaultTemperature : this.defaultTemperature;
    ...
    await gpuFan.apply(this.display);
