      ...
      Prompt.input(`${"Do you want to remove these profiles?".yellow}\n(${"yes".green}/${"no".red})`, answer => {
        ...
        if (answer.toLowerCase() === "yes" || answer.toLowerCase() === "y") {
          ...
          if (profileFilter === undefined && gpuFilter === undefined && tagFilter === undefined) {
            ...
            removeProfileByFilter(configObject, profileFilter, gpuFilter, tagFilter, display);
          }
...
function removeProfileByFilter(config, profileFilter, gpuFilter, tagFilter, display) {
  const gpuSetting = new Gpu(display);
  const gpus = gpuSetting.list();
  ...
  Object.keys(config.gpus)
    ...
    .filter(gpu => gpu.config.profile === profileFilter)
    .filter(gpu => gpuFilter === undefined || gpu.name.indexOf(gpuFilter) > -1)
