...
function NvidiaGpu(display = ":0") {
  return {
    list: function () {
      ...
      const response = executor.execute(`nvidia-settings -c ${display} -q gpus --verbose`);
...
function readFanSpeed(gpus, display = ":0") {
  const executor = new CommandLineExecutor();
  executor.errorHandler(NvidiaErrorHandler);
  const response = executor.execute(`nvidia-settings -c ${display} -q /GPUCurrentFanSpeed`);
