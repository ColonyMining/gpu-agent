...
function Config(display = ":0") {
  return {
    ...
    save: function (selectedGpus, profileName, hashrate) {
      try {
        ...
        filterGpus(gpus, selectedGpus).forEach(gpu => {
          ...
          if (profile[gpu.name] === undefined) {
            profile[gpu.name] = profile[`${gpu.name}${reference}`];
          }
    ...
    apply: async function (selectedGpus, profileName, verbose) {
      const gpuController = new Gpu(display);
