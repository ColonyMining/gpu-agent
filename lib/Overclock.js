...
class Overclock {
  ...
  async current(selectedGpus = "all") {
  ...
  async set(selectedGpus, options, verbose) {
    const gpuController = new Gpu(this.display);
    const gpus = await gpuController.list();
    ...
    const config = new Config();
    config.set(
      filterSelectedGpus.map(gpu => ({ index: gpu.index, name: gpu.name })),
      options
    );
    config.save();
  }

  async save(selectedGpus, profileName, hashrate) {
    const gpuController = new Gpu(this.display);
    const gpus = await gpuController.list();
  ...
  async load(selectedGpus, profileName, verbose) {
    const gpuController = new Gpu(this.display);
    const gpus = await gpuController.list();
  ...
  async reset(selectedGpus, verbose) {
    const gpuController = new Gpu(this.display);
    const gpus = await gpuController.list();
    const overclock = gpuController.overclock();
