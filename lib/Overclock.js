...
function Overclock(display) {
  return {
    ...
    set: async function (selectedGpu, options, verbose) {
      const gpuController = new Gpu(display);
      const gpus = await gpuController.list();
    ...
    load: async function (selectedGpus, profile, verbose) {
      await new Config(display).apply(selectedGpus, profile, verbose);
    },
