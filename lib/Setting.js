...
function setOverclock(selectedGpus = "all", options, verbose, display) {
  ...
  if (options.power) {
    filterSelectedGpus.forEach(gpu => overclock.power(gpu.index, parseInt(options.power)));
  }
  if (options.core) {
    filterSelectedGpus.forEach(gpu => overclock.core(gpu.index, parseInt(options.core)));
  }
  if (options.memory) {
    filterSelectedGpus.forEach(gpu => overclock.memory(gpu.index, parseInt(options.memory)));
  }
  if (options.fan) {
    filterSelectedGpus.forEach(gpu =>
      gpu.fans.forEach(fan => overclock.fan(gpu.index, fan.index, parseInt(options.fan)))
    );
  }
  if (options.lgc) {
    filterSelectedGpus.forEach(gpu => overclock.lockGraphicsClock(gpu.index, parseInt(options.lgc)));
  }
  if (options.lmc) {
    filterSelectedGpus.forEach(gpu => overclock.lockMemoryClock(gpu.index, parseInt(options.lmc)));
  }
