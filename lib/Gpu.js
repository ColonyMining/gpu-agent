...
function listGpus(display = ":0") {
  const output = childProcess.execSync(`nvidia-settings -c ${display} -q gpus --verbose`, { stdio: "ignore" });
...
function readFanSpeed(gpus, display = ":0") {
  const output = childProcess.execSync(`nvidia-settings -c ${display} -q /GPUCurrentFanSpeed`, { stdio: "ignore" });
  const response = output.toString().trim();
...
const GpuOverclock = function (display = ":0") {
  ...
  return {
    fan: function (gpuIndex, fanIndex, speed) {
      commands.push({
        message: `Set fan speed ${speed} to GPU ${gpuIndex}`,
        execute:
    ...
    power: function (gpuIndex, value) {
      commands.push({
        message: `Set power limit ${value} to GPU ${gpuIndex}`,
    ...
    lockGraphicsClock: function (gpuIndex, value) {
      commands.push({
        message: `Set lock graphics clock ${value} to GPU ${gpuIndex}`,
    ...
    resetLockGraphicsClock: function (gpuIndex) {
      commands.push({
        message: `Reset lock graphics clock to GPU ${gpuIndex}`,
    ...
    core: function (gpuIndex, value) {
      commands.push({
        message: `Set graphics clock offset ${value} to GPU ${gpuIndex}`,
    ...
    memory: function (gpuIndex, value) {
      commands.push({
        message: `Set memory transfer rate offset ${value} to GPU ${gpuIndex}`,
    ...
    apply: function (verbose = false) {
      commands.forEach(command => {
        try {
          ...
          if (verbose) {
            console.log(command.message.green);
          }
        } catch (e) {
