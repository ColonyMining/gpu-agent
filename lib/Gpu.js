  ...
  return {
    ...
    apply: function (display = ":0") {
      ...
      const args = assignments
        ...
        .join("");
      const result = childProcess.spawnSync(`nvidia-settings -c ${display} ${args}`);
      if (result.error) {
        console.error("Error applying fan speed", result.stderr.toString("utf-8"));
      }
    }
  };
}
  ...
  return {
    ...
    apply: function () {
      commands.forEach(command => {
        const result = childProcess.spawnSync(command.execute);
        if (result.error) {
          console.error(command.errorMessage.red, result.stderr.toString("utf-8"));
        } else {
